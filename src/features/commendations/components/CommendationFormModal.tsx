...
interface Props {
  onSubmit: (data: Omit<Commendation, "id">) => void;
  onCancel: () => void;
}

function CommendationFormModal({ onSubmit, onCancel }: Props) {
  const [formData, setFormData] = useState({
    title: "",
    description: "",
    type: "MEDAL" as Commendation["type"],
    level: "INDIVIDUAL" as Commendation["level"],
    date: new Date().toISOString().split("T")[0],
    recipientName: "",
    unit: "",
    criteria: "",
    approvedBy: "",
    status: "PENDING" as Commendation["status"]
  });
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        ...
        <input
          ...
          placeholder="Ví dụ: Huy chương Chiến sĩ Xuất sắc"
          required
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => setFormData({ ...formData, status: e.target.value as Commendation["status"] })}
            className="w-full rounded-lg border-2 border-gray-200 px-4 py-2 focus:border-forest focus:outline-none"
          >
            <option value="PENDING">Chờ duyệt</option>
            <option value="APPROVED">Đã duyệt</option>
            <option value="AWARDED">Đã trao</option>
          </select>
        </div>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full rounded-lg border-2 border-gray-200 px-4 py-2 focus:border-forest focus:outline-none"
          rows={2}
          placeholder="Liệt kê các tiêu chí đã đạt được..."
        />
      </div>
...
export default CommendationFormModal;